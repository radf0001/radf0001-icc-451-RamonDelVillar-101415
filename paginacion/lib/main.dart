      ...
      ApiService.getPokemons().then((value)async{
        if(value != null){
          for(Result m in value){
            await dbHelper.insertData(PokemonDb(idName: "${m.url.split('/')[6]} ${m.name}"));
          }
        }
      });
    }
        ...
        child: MasonryGridView.builder(
          ...
          itemBuilder: (context, index){
            ...
            return Padding(
              ...
              child: InkWell(
                onTap: () async {
                  valueChoose == "all" ? await fetchPokemonDetails(pokemonsResult[index].url.split('/')[6]) : await fetchPokemonDetails(pokemonsResult[index].pokemon.url.split('/')[6]);
                  Navigator.of(context).push(
                    MaterialPageRoute(
                      builder: (context) => valueChoose == "all" ? DetallePokemon(pokemonNumber: pokemonsResult[index].url.split('/')[6], pokemonName: pokemonsResult[index].name, pokemonDetails: pokemonDetails,) :
                      DetallePokemon(pokemonNumber: pokemonsResult[index].pokemon.url.split('/')[6], pokemonName: pokemonsResult[index].pokemon.name, pokemonDetails: pokemonDetails,),
                    ),
                  );
                },
                child: Container(
